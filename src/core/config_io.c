...
static BongoTheme parse_theme(const char *value) {
    if (value && strcmp(value, "light") == 0) return BONGO_THEME_LIGHT;
    if (value && strcmp(value, "dark") == 0) return BONGO_THEME_DARK;
    return BONGO_THEME_AUTO;
}

static BongoLanguage parse_language(const char *value) {
    const char *names[] = {"en-US", "zh-CN", "zh-TW", "pt-BR", "vi-VN"};
    if (value) for (int i = 0; i <= BONGO_LANG_VI_VN; ++i) {
        if (strcmp(value, names[i]) == 0) return (BongoLanguage)i;
    }
    return BONGO_LANG_EN_US;
}

static BongoModelMode parse_mode(const char *value) {
    if (value && strcmp(value, "keyboard") == 0) return BONGO_MODE_KEYBOARD;
    if (value && strcmp(value, "gamepad") == 0) return BONGO_MODE_GAMEPAD;
    return BONGO_MODE_STANDARD;
}

static void read_model(yyjson_val *obj, BongoModelOptions *value) {
...
static void read_window(yyjson_val *obj, BongoWindowOptions *value) {
    if (!yyjson_is_obj(obj)) return;
...
static void read_app(yyjson_val *obj, BongoAppOptions *value) {
    if (!yyjson_is_obj(obj)) return;
...
static void read_shortcuts(yyjson_val *obj, BongoShortcutOptions *value) {
    if (!yyjson_is_obj(obj)) return;
...
static void read_behaviors(yyjson_val *array, BongoConfig *config) {
    ...
    yyjson_arr_foreach(array, index, count, item) {
        ...
        if (!id || !shortcut || config->behavior_shortcut_count >= BONGO_BEHAVIOR_CAP) continue;
...
BongoResult bongo_config_load(const char *path, BongoConfig *config, BongoError *error) {
    if (!path || !config) return BONGO_ERROR_ARGUMENT;
    bongo_config_defaults(config);
    if (!bongo_path_is_file(path)) return BONGO_OK;
    ...
    if (file) fclose(file);
    if (!doc) {
        bongo_error_set(error, BONGO_ERROR_FORMAT, "Invalid settings JSON: %s",
    ...
    yyjson_doc_free(doc);
    bongo_config_validate(config);
    return BONGO_OK;
}

static void write_model(yyjson_mut_doc *doc, yyjson_mut_val *obj, const BongoModelOptions *v) {
...
static void write_window(yyjson_mut_doc *doc, yyjson_mut_val *obj, const BongoWindowOptions *v) {
...
static void write_app(yyjson_mut_doc *doc, yyjson_mut_val *obj, const BongoAppOptions *v) {
    yyjson_mut_obj_add_bool(doc, obj, "autostart", v->autostart);
    yyjson_mut_obj_add_bool(doc, obj, "trayVisible", v->tray_visible);
    yyjson_mut_obj_add_strcpy(doc, obj, "theme", bongo_theme_name(v->theme));
    yyjson_mut_obj_add_strcpy(doc, obj, "language", bongo_language_name(v->language));
}

static void write_shortcuts(yyjson_mut_doc *doc, yyjson_mut_val *obj,
    const BongoShortcutOptions *value) {
...
BongoResult bongo_config_save(const char *path, const BongoConfig *config, BongoError *error) {
    ...
    if (!doc) return BONGO_ERROR_MEMORY;
    ...
    yyjson_mut_obj_add_strcpy(doc, root, "currentMode", bongo_mode_name(config->current_mode));
    char temporary[BONGO_PATH_CAP + 8];
    snprintf(temporary, sizeof(temporary), "%s.tmp", path);
    yyjson_write_err json_error = {0};
    FILE *file = bongo_file_open(temporary, "wb");
    bool written = file && yyjson_mut_write_fp(file, doc,
        YYJSON_WRITE_PRETTY, NULL, &json_error);
    if (file && fclose(file) != 0) written = false;
    yyjson_mut_doc_free(doc);
    if (!written) {
        bongo_error_set(error, BONGO_ERROR_IO, "Cannot write settings: %s",
            json_error.msg ? json_error.msg : "cannot open file");
        return BONGO_ERROR_IO;
    }
    if (!sync_file(temporary)) {
        bongo_file_remove(temporary);
        bongo_error_set(error, BONGO_ERROR_IO, "Cannot flush settings file");
        return BONGO_ERROR_IO;
    }
    if (!bongo_file_replace(temporary, path, true)) {
        bongo_file_remove(temporary);
        bongo_error_set(error, BONGO_ERROR_IO, "Cannot replace settings file");
        return BONGO_ERROR_IO;
    }
#ifndef _WIN32
    if (!sync_parent(path)) {
        bongo_error_set(error, BONGO_ERROR_IO, "Cannot flush settings directory");
        return BONGO_ERROR_IO;
    }
#endif
    return BONGO_OK;
