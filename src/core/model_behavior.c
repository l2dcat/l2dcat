...
static bool add_behavior(BongoBehaviorCatalog *catalog, const BongoModelEntry *model,
    BongoBehaviorKind kind, const char *group, int index, const char *label,
    const char *sound) {
    if (catalog->count >= BONGO_BEHAVIOR_CAP) return false;
    BongoBehaviorEntry *entry = &catalog->entries[catalog->count++];
    entry->kind = kind;
    entry->index = index;
    snprintf(entry->group, sizeof(entry->group), "%s", group ? group : "");
    snprintf(entry->label, sizeof(entry->label), "%s", label ? label : "");
    if (kind == BONGO_BEHAVIOR_MOTION)
    ...
    if (sound && *sound)
        bongo_path_join(entry->sound, sizeof(entry->sound), model->directory, sound);
    return true;
}

static bool read_motions(BongoBehaviorCatalog *catalog, const BongoModelEntry *model,
    yyjson_val *motions) {
    ...
    yyjson_obj_foreach(motions, group_index, group_count, group_key, items) {
        ...
        yyjson_arr_foreach(items, index, count, item) {
            ...
            if (!add_behavior(catalog, model, BONGO_BEHAVIOR_MOTION, group,
...
static bool read_expressions(BongoBehaviorCatalog *catalog,
    const BongoModelEntry *model, yyjson_val *expressions) {
    ...
    yyjson_arr_foreach(expressions, index, count, item) {
        ...
        if (!add_behavior(catalog, model, BONGO_BEHAVIOR_EXPRESSION, NULL,
...
BongoResult bongo_behaviors_load(BongoBehaviorCatalog *catalog,
    ...
    if (!catalog || !model) return BONGO_ERROR_ARGUMENT;
    memset(catalog, 0, sizeof(*catalog));
    char path[BONGO_PATH_CAP];
    if (!bongo_path_join(path, sizeof(path), model->directory, model->setting_file))
    ...
    if (file) fclose(file);
    if (!document) {
        bongo_error_set(error, BONGO_ERROR_FORMAT, "Cannot read model setting: %s",
    ...
    if (!ok) {
        bongo_error_set(error, BONGO_ERROR_FORMAT, "Too many model behaviors");
        return BONGO_ERROR_FORMAT;
    }
    return BONGO_OK;
}
