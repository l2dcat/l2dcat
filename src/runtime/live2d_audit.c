...
static void input(BongoApp *app, BongoInputKind kind, const char *name, float value) {
    BongoInputEvent event = {.kind = kind, .value = value};
    snprintf(event.name, sizeof(event.name), "%s", name);
    bongo_app_apply_input(app, &event);
}

static bool motion(BongoApp *app, const char *scenario) {
    if (strcmp(scenario, "motion-0") == 0)
        return bongo_live2d_start_motion(app->live2d, "CAT_motion", 0);
    if (strcmp(scenario, "motion-1") == 0)
        return bongo_live2d_start_motion(app->live2d, "CAT_motion", 1);
    if (strncmp(scenario, "expression-", 11) == 0)
        return bongo_live2d_set_expression(app->live2d, atoi(scenario + 11));
    return true;
}

static bool apply(BongoApp *app, const char *scenario) {
    if (strcmp(scenario, "mirror") == 0) app->config.model.mirror = true;
    else if (strcmp(scenario, "key-left") == 0)
        input(app, BONGO_INPUT_KEY_DOWN, "KeyA", 1.0f);
    else if (strcmp(scenario, "key-right") == 0)
        input(app, BONGO_INPUT_KEY_DOWN, "RightArrow", 1.0f);
    else if (strcmp(scenario, "key-left-release") == 0) {
        input(app, BONGO_INPUT_KEY_DOWN, "KeyA", 1.0f);
        input(app, BONGO_INPUT_KEY_UP, "KeyA", 0.0f);
    } else if (strcmp(scenario, "keys-both-release") == 0) {
        input(app, BONGO_INPUT_KEY_DOWN, "KeyA", 1.0f);
        input(app, BONGO_INPUT_KEY_DOWN, "RightArrow", 1.0f);
        input(app, BONGO_INPUT_KEY_UP, "KeyA", 0.0f);
        input(app, BONGO_INPUT_KEY_UP, "RightArrow", 0.0f);
        ...
        input(app, BONGO_INPUT_MOUSE_DOWN, "Left", 1.0f);
    else if (strcmp(scenario, "mouse-right") == 0)
        input(app, BONGO_INPUT_MOUSE_DOWN, "Right", 1.0f);
    else if (strcmp(scenario, "gamepad-buttons") == 0) {
        input(app, BONGO_INPUT_GAMEPAD_BUTTON, "DPadLeft", 1.0f);
        input(app, BONGO_INPUT_GAMEPAD_BUTTON, "South", 1.0f);
    } else if (strcmp(scenario, "gamepad-sticks") == 0) {
        input(app, BONGO_INPUT_GAMEPAD_AXIS, "LeftStickX", .75f);
        input(app, BONGO_INPUT_GAMEPAD_AXIS, "LeftStickY", -.5f);
        input(app, BONGO_INPUT_GAMEPAD_AXIS, "RightStickX", -.65f);
        input(app, BONGO_INPUT_GAMEPAD_AXIS, "RightStickY", .5f);
        input(app, BONGO_INPUT_GAMEPAD_BUTTON, "LeftThumb", 1.0f);
        input(app, BONGO_INPUT_GAMEPAD_BUTTON, "RightThumb", 1.0f);
    } else return motion(app, scenario);
    app->dirty = true;
    return true;
}

static void parameter(FILE *file, BongoApp *app, const char *id) {
    BongoParameterRange range;
    if (bongo_live2d_parameter(app->live2d, id, &range))
        fprintf(file, "parameter.%s=%.4f [%.4f,%.4f]\n", id,
            range.value, range.minimum, range.maximum);
    else fprintf(file, "parameter.%s=unavailable\n", id);
}

static bool value(BongoApp *app, const char *id, float *output) {
    BongoParameterRange range;
    if (!bongo_live2d_parameter(app->live2d, id, &range)) return false;
    if (output) *output = range.value;
    return true;
}

static bool active(BongoApp *app, const char *id) {
    float current = 0.0f;
    return value(app, id, &current) && current > 0.5f;
}

static bool signed_value(BongoApp *app, const char *id, bool positive) {
    float current = 0.0f;
    return value(app, id, &current) && (positive ? current > 0.05f : current < -0.05f);
}

static bool assertions(BongoApp *app, const char *scenario, bool operation) {
...
void bongo_live2d_audit_run(BongoApp *app) {
    ...
    if (!bongo_path_join(path, sizeof(path), app->data_root, "live2d-audit.txt")) return;
    ...
    fprintf(file, "scenario=%s\nmodel=%s\nmode=%s\noperation=%s\nassertions=%s\n",
        ...
        bongo_mode_name(app->config.current_mode), result ? "accepted" : "rejected",
