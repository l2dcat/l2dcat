...
static void http_error(L2DCatError *error, const char *message) {
    l2dcat_error_set(error, L2DCAT_ERROR_IO, "%s (Windows error %lu)",
...
static bool request_parts(wchar_t *url, wchar_t *host, size_t host_capacity,
    wchar_t *path, size_t path_capacity, INTERNET_PORT *port, L2DCatError *error) {
    ...
    if (!WinHttpCrackUrl(url, 0, 0, &parts) || parts.nScheme != INTERNET_SCHEME_HTTPS) {
        l2dcat_error_set(error, L2DCAT_ERROR_FORMAT, "Update URL must use HTTPS"); return false;
    }
    if (parts.dwHostNameLength >= host_capacity ||
        parts.dwUrlPathLength + parts.dwExtraInfoLength >= path_capacity) {
        l2dcat_error_set(error, L2DCAT_ERROR_FORMAT, "Update URL is too long"); return false;
    }
...
L2DCatResult l2dcat_platform_download(const char *url, const char *destination,
    ...
    wchar_t *url_wide = wide(url), host[256], path[L2DCAT_PATH_CAP]; INTERNET_PORT port;
    if (!url_wide || !request_parts(url_wide, host, 256, path, L2DCAT_PATH_CAP, &port, error)) {
        free(url_wide); return L2DCAT_ERROR_FORMAT;
    }
    HINTERNET session = WinHttpOpen(L"l2dcat/" L"1.1.0", WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY,
    ...
    if (ok) {
        ...
        if (ok && status != 200) {
            l2dcat_error_set(error, L2DCAT_ERROR_IO, "Update server returned HTTP %lu",
    ...
    if (ok && content_length(request, &total) && limit && total > limit) {
        l2dcat_error_set(error, L2DCAT_ERROR_IO, "Update download exceeds size limit"); ok = false;
    }
    FILE *file = ok ? l2dcat_file_open(destination, "wb") : NULL;
    if (ok && !file) { l2dcat_error_set(error, L2DCAT_ERROR_IO, "Cannot create update file"); ok = false; }
    ...
    while (ok) {
        ...
        if ((limit && received > limit) || fwrite(buffer, 1, count, file) != count) {
            l2dcat_error_set(error, L2DCAT_ERROR_IO, "Cannot write update download"); ok = false; break;
        }
    ...
    free(url_wide);
    return ok ? L2DCAT_OK : L2DCAT_ERROR_IO;
}
#endif
